 ...
 * kimxilxyong@gmail.com 17/06/2021
 *
 * PWM decoding of RC Receiver with failsafe
 *  
 * Originally developed by Kelvin Nelson for the Uno, posted on https://create.arduino.cc/projecthub/kelvineyeone/read-pwm-decode-rc-receiver-input-and-apply-fail-safe-6b90eb
 * As Kelvin did not specify a license, but posted it on a public forum, I just decided to give it a license together with
 * added support for the Mega Family, developed on Mega 2560
 * 
 * If you see any issue with the new license for this code, please email me
 *  
 */
...
// void           setup_ports_and_pins()     initialise the PWM measurement using pin change interrupts
...
// EXAMPLE USE OF GENERIC PWM FUNCTIONS:
 ...
 if (PWM_read_Channel(1)){                                      // if a new pulse is detected on channel 1
...
// Mega 2560
// PWM input pins, any of the following pins can be used: digital 10 - 13, 50 - 53 or analog A8 - A15 

//const int pwmPIN[]={50,51,52,53,A8,A9,A10,A11,A12,A13,A14,A15}; // an array to identify the PWM input pins (the array can be any length) 
                                  // first pin is channel 1, second is channel 2...etc
const int pwmPIN[]={A8,A9,A10,A11,A12}; // an array to identify the PWM input pins (the array can be any length)                                   
...
volatile boolean RC_Section_complete;           // flag when all RC receiver channels have received a new pulse
...
volatile int bitsToSetPCICR = 0;
volatile int bitsToSetPCMSK0 = 0;
volatile int bitsToSetPCMSK1 = 0;
volatile int bitsToSetPCMSK2 = 0;

#define RC_DEBUG 1

//DEBUG
volatile unsigned int dPINB = 0;
volatile unsigned int dPINJ = 0;
volatile unsigned int dPINK = 0;
...
void setup()  {
  ...
  Serial.print("num_ch"); Serial.print(": "); Serial.println(num_ch);
  Serial.print("RC_inputs"); Serial.print(": "); Serial.println(RC_inputs);
  Serial.print("MCUCR"); Serial.print(": "); Serial.println(MCUCR, BIN);                
...
void loop() {
    if (is_Section_completed()) {
      if (readyToPrint()) {
        ...
        if (PWM_read_Channel(1)) {                                      // if a new pulse is detected on channel 1
          ...
          Serial.print(PWM_period(),0);Serial.print(" uS  ");     
          Serial.print(PWM_freq());Serial.print(" Hz  ");
          Serial.print(PWM());Serial.print(" PW");
        }
        Serial.println("");
      }
    }
  ...
  /*  if (dPINK != 0) {
      Serial.print("dPINK "); Serial.print(dPINK, BIN);Serial.print(" / ");
      dPINK = 0;
      Serial.print("dPINB "); Serial.print(dPINB, BIN);Serial.print(" / ");
      Serial.print("dPINJ "); Serial.print(dPINJ, BIN);Serial.print(" / ");
      
      Serial.print("dIO0 "); Serial.print(dIO0);Serial.print(" / ");
      Serial.print("dIO1 "); Serial.print(dIO1);Serial.print(" / ");
      Serial.print("dIO2 "); Serial.println(dIO2);                    
    } */
}
...
bool readyToPrint() {
  ...
  // Print 4 times per second, for viewing as
  // scrolling numbers in the Arduino Serial Monitor
  nowMillis = millis();
  if (nowMillis - thenMillis > 250) {
...
void pwmPIN_to_port_and_mask() {
  
  for (int i = 0; i < num_ch; i++){
    
    #if defined __AVR_ATmega2560__  || defined __AVR_ATmega2561__ 

        // convert the pin number (i.e. pin 11 or pin A0) to the pin position in the port register. There is most likely a better way of doing this using a macro...
        // (Reading the pin state directly from the port registers speeds up the code in the ISR)
        
        if(pwmPIN[i] == 53 || pwmPIN[i] == A8 || pwmPIN[i] == 15)       pwmPIN_reg[i] = 0b00000001;
        else if(pwmPIN[i] == 52 || pwmPIN[i] == A9 || pwmPIN[i] == 14)  pwmPIN_reg[i] = 0b00000010;
        else if(pwmPIN[i] == 51 || pwmPIN[i] == A10 )                   pwmPIN_reg[i] = 0b00000100;
        else if(pwmPIN[i] == 50 || pwmPIN[i] == A11)                    pwmPIN_reg[i] = 0b00001000;
        else if(pwmPIN[i] == 10 || pwmPIN[i] == A12 )                   pwmPIN_reg[i] = 0b00010000;
        else if(pwmPIN[i] == 11 || pwmPIN[i] == A13 )                   pwmPIN_reg[i] = 0b00100000;
        else if(pwmPIN[i] == 12 || pwmPIN[i] == A14 )                   pwmPIN_reg[i] = 0b01000000;
        else if(pwmPIN[i] == 13 || pwmPIN[i] == A15 )                   pwmPIN_reg[i] = 0b10000000;    
    
        // determine which port and therefore ISR (PCINT0_vect, PCINT1_vect or PCINT2_vect) each pwmPIN belongs to on the Mega family
        
        if ((pwmPIN[i] >= 10 && pwmPIN[i] <= 13) || (pwmPIN[i] >= 50 && pwmPIN[i] <= 53)) {
          pwmPIN_port[i] = 0;    // pin belongs to PCINT0_vect (PORT B)
          bitsToSetPCICR |= (1<<(pwmPIN_port[i]));
          bitsToSetPCMSK0 |= pwmPIN_reg[i];
          // Data Direction Register - all INPUT, if you want output on this port you have to add this functionality yourself
          ...
          pwmPIN_port[i] = 1;    // pin belongs to PCINT1_vect (PORT J)
          bitsToSetPCICR |= (1<<(pwmPIN_port[i]));
          bitsToSetPCMSK1 |= pwmPIN_reg[i];
          // Data Direction Register - all INPUT, if you want output on this port you have to add this functionality yourself
          // This is also a safety measure, as i shot an IR Sensor by connecting its signal to an OUTPUT pin
          DDRJ = B00000000;
          #if defined RC_DEBUG
          ...
          pwmPIN_port[i] = 2;    // pin belongs to PCINT2_vect (PORT K)
          bitsToSetPCICR |= (1<<(pwmPIN_port[i]));
          bitsToSetPCMSK2 |= pwmPIN_reg[i];
          // Data Direction Register - all INPUT, if you want output on this port you have to add this functionality yourself
          // This is also a safety measure, as i shot an IR Sensor by connecting its signal to an OUTPUT pin
          DDRK = B00000000;
          #if defined RC_DEBUG
    ...
    #else
      #if defined __AVR_ATmega1280__ || defined __AVR_ATmega1281__ || defined __AVR_ATmega640__
        #error This board is not supported/tested yet - enable the code below at your own risk
      #else
        // Uno & others
        ...
        else if (pwmPIN[i] >= A0 && pwmPIN[i] <= A5) {
...
void setup_ports_and_pins() {
  
  // Enable global interrupts
  SREG |= (1<<7);
  
  pwmPIN_to_port_and_mask();                    // determines the port for each input pin
                                                // pwmPIN_to_port() also converts the pin number in pwmPIN[] (i.e. pin 11 or pin A0) to the pin position in the port register (i.e. 0b00000001) for use in the ISRs.
  // Control Register
  PCICR = bitsToSetPCICR;
  // Clear Flag Register
  PCIFR = bitsToSetPCICR;
  
  // Mask registers
  // CODE USED TO TURN ON THE INTERRUPTS ON THE RELEVANT PINS
  PCMSK0 = bitsToSetPCMSK0;
  PCMSK1 = bitsToSetPCMSK1;
  PCMSK2 = bitsToSetPCMSK2;
...
// Mega
// the PCINT0_vect (B port register) reacts to any changes on pins 10-13 & 50-53
// the PCINT1_vect (K port register)          ""        ""         A8-A15
// the PCINT2_vect (J port register)          ""        ""         14-15   (danger, dont mess with communication pins)
...
// Mega 2560
// READ INTERRUPTS ON PINS 10-13 & 50-53: ISR routine detects which pin has changed, and returns PWM pulse width, and pulse repetition period.
...
ISR(PCINT0_vect){                                                 // this function will run if a pin change is detected on portB
  ...
  for (int i = 0; i < num_ch; i++) {                              // run through each of the channels
    if (pwmPIN_port[i] == 0) {                                    // if the current channel belongs to portB
      ...
      if(prev_pinState[i] == 0 && pinPort & pwmPIN_reg[i]){          // and the pin state has changed from LOW to HIGH (start of pulse)
      ...
      else if (prev_pinState[i] == 1 && !(pinPort & pwmPIN_reg[i])){ // or the pin state has changed from HIGH to LOW (end of pulse)
        ...
        if(i+1 == RC_inputs) RC_Section_complete = true;                  
        dIO0 = 5;
      }
    }
  }
...
// Mega 2560
// READ INTERRUPTS ON PINS 14 - 15: ISR routine detects which pin has changed, and returns PWM pulse width, and pulse repetition period.

ISR(PCINT1_vect){                                                 // this function will run if a pin change is detected on portC / portJ
  ...
  for (int i = 0; i < num_ch; i++) {                              // run through each of the channels
    if (pwmPIN_port[i] == 1) {                                    // if the current channel belongs to portC / portJ
       if (prev_pinState[i] == 0)
         dIO1 = 1;
      if (pinPort)
         dIO1= 2;
...
// Mega 2560
// READ INTERRUPTS ON PINS A8 - A15: ISR routine detects which pin has changed, and returns PWM pulse width, and pulse repetition period.

ISR(PCINT2_vect){                                                 // this function will run if a pin change is detected on portD / portK
  ...
  for (int i = 0; i < num_ch; i++) {                              // run through each of the channels
    if (pwmPIN_port[i] == 2) {                                    // if the current channel belongs to portD / portK
      if (prev_pinState[i] == 0)
         dIO2 = 1;
      if (pinPort)
         dIO2 = 2;
...
boolean is_Section_completed() {
  if (RC_Section_complete) {
    RC_Section_complete = false;                          // reset the flag
    return true;
  } else {
    return false;
  }
...
void print_all_Channels(){                             // display the raw RC Channel PWM Inputs
...
boolean PWM_read_Channel(int CH){
